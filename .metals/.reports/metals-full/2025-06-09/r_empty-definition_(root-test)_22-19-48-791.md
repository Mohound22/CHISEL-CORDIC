error id: file://<WORKSPACE>/src/test/scala/CORDIC/ChiselVsScalaModelTests.scala:magnitudeOut
file://<WORKSPACE>/src/test/scala/CORDIC/ChiselVsScalaModelTests.scala
empty definition using pc, found symbol in pc: magnitudeOut
...
class TrigChiselVsScalaTest extends ChiselVsScalaTester {
  ...
  def runTest(
    ...
    if (mode == CordicSimplifiedConstants.Mode.SinCos) {
      ...
      fixedToDouble(dut.io.magnitude@@Out.peek().litValue) should be (fixedToDouble(model.magnitude) +- tolerance)
    }
    dut.clock.step(2)
  }
