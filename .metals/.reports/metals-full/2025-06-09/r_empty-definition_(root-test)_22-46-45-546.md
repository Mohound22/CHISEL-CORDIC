error id: file://<WORKSPACE>/src/test/scala/CORDIC/ChiselVsScalaModelTests.scala:atanhOut
file://<WORKSPACE>/src/test/scala/CORDIC/ChiselVsScalaModelTests.scala
empty definition using pc, found symbol in pc: atanhOut
  ...
  def runHyperTest(
    ...
    mode match {
      ...
      case HyperCordicConstants.Mode.Exponential =>
        println(s"  Model exp: ${fixedToDouble(model.exp)} (${model.exp})")
        println(s"  Model expNeg: ${fixedToDouble(model.expNeg)} (${model.expNeg})")
      case HyperCordicConstants.Mode.NaturalLog =>
        println(s"  Model ln: ${fixedToDouble(model.ln)} (${model.ln})")
    }
    ...
    mode match {
      case HyperCordicConstants.Mode.SinhCosh =>
        ...
        val dutSinh = dut.io.sinhOut.peek().litValu
      ...
      case HyperCordicConstants.Mode.Exponential =>
        ...
        println(s"  DUT exp: ${fixedToDouble(dutExp)} ($dutExp)")
        println(s"  DUT expNeg: ${fixedToDouble(dutExpNeg)} ($dutExpNeg)")
      ...
      case HyperCordicConstants.Mode.NaturalLog =>
        ...
        println(s"  DUT ln: ${fixedToDouble(dutLn)} ($dutLn)")
