error id: file://<WORKSPACE>/src/main/scala/CORDIC/CORDICscalaModel.scala:scala/math/package.pow().
file://<WORKSPACE>/src/main/scala/CORDIC/CORDICscalaModel.scala
empty definition using pc, found symbol in pc: 
found definition using semanticdb; symbol scala/math/package.pow().
    ...
    if (trueArcTanfalseSinCos) { // Arctan, take in X and Y and spit out angle
      ...
      for (i <- 0 until cycleCount) {
        ...
        if (yPrime < 0) { // Positive angle added
          ...
          xPrime = xOld - direction * yOld * math.pow(2, -i) // SWAP TO >> WHEN FIXED DECIMAL
          yPrime = yOld + direction * xOld * math.pow(2, -i)
          totalTheta += deltaTheta
        } else { // Negative angle added
          xPrime = xOld - yOld * math.pow(2, -i) // SWAP TO >> WHEN FIXED DECIMAL
          yPrime = yOld + xOld * math.@@pow(2, -i)
          totalTheta -= deltaTheta
        }
