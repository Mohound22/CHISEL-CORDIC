error id: file://<WORKSPACE>/src/test/scala/CORDIC/scalaHyperTestIterable.scala:scala/math/package.cosh().
file://<WORKSPACE>/src/test/scala/CORDIC/scalaHyperTestIterable.scala
empty definition using pc, found symbol in pc: 
found definition using semanticdb; symbol scala/math/package.cosh().
...
class HyperCordicModelTest extends AnyFlatSpec with Matchers {
  ...
  it should "handle inputs for SinhCosh leading to results near fixed-point limits" in {
    // Choose theta such that cosh(theta) or sinh(theta) is large but representable
    // e.g. integerBits = 3 allows values up to approx 2^(3) = 8.
    // cosh(2) approx 3.76, sinh(2) approx 3.62
    // If we use theta = 1.1
    val testTheta = math.pi/2
    runTest(theta = testTheta, mode = SinhCosh)
    // Increase precision tolerance for larger values and potential fixed-point inaccuracies
    fixedToDouble(model.cosh) should be (cosh@@(testTheta) +- precision * 10) 
    fixedToDouble(model.sinh) should be (sinh(testTheta) +- precision * 10)
    // Verify cosh^2 - sinh^2 = 1
    val ch = fixedToDouble(model.cosh)
    val sh = fixedToDouble(model.sinh)
    (ch*ch - sh*sh) should be (1.0 +- precision * 20) // Error can compound
  }
  ...
  it should "satisfy cosh^2(theta) - sinh^2(theta) ~= 1 with correction enabled for SinhCosh" in {
    ...
    val testThetas = Seq(0.0, 0.3, 0.8, -0.5, 1.0, 1.5) // Added more test thetas, including 0
  ...
  it should "satisfy (K_h*cosh(theta))^2 - (K_h*sinh(theta))^2 ~= K_h^2 with correction disabled for SinhCosh" in {
    ...
    val testThetas = Seq(0.0, 0.3, 0.8, -0.5, 1.0, 1.2) // Max theta around 1.2-1.5 for 3 integer bits
    for (theta_val <- testThetas) {
