error id: file://<WORKSPACE>/src/main/scala/CORDIC/linearCHISEL.scala:
file://<WORKSPACE>/src/main/scala/CORDIC/linearCHISEL.scala
...
offset: 3677
uri: file://<WORKSPACE>/src/main/scala/CORDIC/linearCHISEL.scala
text:
```scala
package CORDIC
...
object LinearCordicConstants {
  ...
  // Approximate range limit for z_cordic (multiplier) or quotient.
  val CORDIC_RANGE_LIMIT_R: Double = 1.99
}

class LinearCordic(val width: Int, val cycleCount: Int, val integerBits: Int = 3) extends Module {
  ...
  def clamp(value: SInt, targetWidth: Int): SInt = {
    ...
    val max_val_extended = maxVal_target.sext(value_w)
    val min_val_extended = minVal_target.sext(value_w)

    Mux(value > max_val_extended, maxVal_target,
      Mux(value < min_val_extended, minVal_target,
        value(targetWidth - 1, 0).asSInt)) // Truncate/s@@elect lower bits
  }
  ...
  switch(state) {
    ...
    is(s.done) {
      ...
      when(currentMode_reg === Mode.Multiply) {
        ...
        var finalQuotient_temp = z_reg << k

        when(originalDivisorIsNegative_reg) {
          finalQuotient_temp = -finalQuotient_temp
        }
        quotientResult_reg := clamp(finalQuotient_temp, width)
      }
